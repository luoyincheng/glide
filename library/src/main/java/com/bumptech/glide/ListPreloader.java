...
/**
 * Loads a few resources ahead in the direction of scrolling in any {@link AbsListView} so that images are in the memory cache just before the corresponding view in created in the list. Gives the
 ...
 * AbsListView#setOnScrollListener(android.widget.AbsListView.OnScrollListener)}, or have its corresponding methods called from another {@link android.widget.AbsListView.OnScrollListener} to
 * function.
 *
...
public class ListPreloader<T> implements AbsListView.OnScrollListener {
   ...
   /**
    * Constructor for {@link com.bumptech.glide.ListPreloader} that accepts interfaces for providing the dimensions of images to preload, the list of models to preload for a given position, and the
    * request to use to load images.
    *
   ...
   public interface PreloadModelProvider<U> {
       ...
       * exactly the same size and set of options as the {@link RequestBuilder} used when the ``View`` is bound. You may need to specify a size in both places to ensure that the width and height match
       * exactly. If so, you can use {@link com.bumptech.glide.request.RequestOptions#override(int, int)} to do so.
       *
