 ...
 * com.bumptech.glide.load.model.ModelLoader}. {@link #loadData(com.bumptech.glide.Priority, com.bumptech.glide.load.data.DataFetcher.DataCallback)} may or may not be called for any given load
 * depending on whether or not the corresponding resource is cached. Cancel also may or may not be called. If {@link #loadData(com.bumptech.glide.Priority,
...
public interface DataFetcher<T> {
    ...
    * here. Any third party libraries called must be thread safe (or move the work to another thread) since this method will be called from a thread in a {@link java.util.concurrent.ExecutorService}
   ...
   /**
    * A method that will be called when a load is no longer relevant and has been cancelled. This method does not need to guarantee that any in process loads do not finish. It also may be called
    * before a load starts or after it finishes.
    *
