...
/**
 * A base class to convert byte arrays to input streams so they can be decoded. This class is abstract because there is no simple/quick way to generate an id from the bytes themselves, so subclass
 * must include an id.
 *
...
public class ByteArrayLoader<Data> implements ModelLoader<byte[], Data> {
   ...
   private static class Fetcher<Data> implements DataFetcher<Data> {
      ...
      /**
       * @param model We really ought to copy the model, but doing so can be hugely expensive and/or lead to OOMs. In practice it's unlikely that users would pass an array into Glide and then mutate
       *              it.
       */
