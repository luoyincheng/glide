...
/**
 * Wraps an existing {@link InputStream} and <em>buffers</em> the input. Expensive interaction with the underlying input stream is minimized, since most (smaller) requests can be satisfied by
 * accessing the buffer alone. The drawback is that some extra space is required to hold the buffer and that copying takes place when filling that buffer, but this is usually outweighed by the
...
public class RecyclableBufferedInputStream extends FilterInputStream {
   ...
   /**
    * Returns an estimated number of bytes that can be read or skipped without blocking for more input. This method returns the number of bytes available in the buffer plus those available in the
    * source stream, but see {@link InputStream#available} for important caveats.
    *
   ...
   /**
    * Sets a mark position in this stream. The parameter {@code readlimit} indicates how many bytes can be read before a mark is invalidated. Calling {@link #reset()} will reposition the stream back
    * to the marked position if {@code readlimit} has not been surpassed. The underlying buffer may be increased in size to allow {@code readlimit} number of bytes to be supported.
    *
   ...
   /**
    * Reads at most {@code byteCount} bytes from this stream and stores them in byte array {@code buffer} starting at offset {@code offset}. Returns the number of bytes actually read or -1 if no bytes
    * were read and the end of the stream was encountered. If all the buffered bytes have been used, a mark has not been put and the requested number of bytes is larger than the receiver's buffer
    * size, this implementation bypasses the buffer and simply places the results directly into {@code buffer}.
    *
