...
/**
 * Generates {@link com.bumptech.glide.load.data.DataFetcher DataFetchers} from original source data using registered {@link com.bumptech.glide.load.model.ModelLoader ModelLoaders} and the model
...
class SourceGenerator implements DataFetcherGenerator, DataFetcherGenerator.FetcherReadyCallback {
   ...
   void onDataReadyInternal(LoadData<?> loadData, Object data) {
      ...
      if (data != null && diskCacheStrategy.isDataCacheable(loadData.fetcher.getDataSource())) {
         PrettyLogger.glideFlow("缓存数据 切换到Glide自身的线程");
         dataToCache = data;
         // We might be being called back on someone else's thread. Before doing anything, we should
         // reschedule to get back onto Glide's thread.
         cb.reschedule();
      } else {
         PrettyLogger.glideFlow("不缓存数据 直接回调 FetcherReadyCallback.onDataFetcherReady");
