 ...
 * gets into a state where a high percentage of {@link Bitmap} allocations trigger a stop the world GC. We try to detect whether or not a GC has occurred by only allowing our allocator to run for a
 * limited number of milliseconds. Since the allocations themselves very fast, a GC is the most likely reason for a substantial delay. If we detect our allocator has run for more than our limit, we
 * assume a GC has occurred, stop the current allocations, and try again after a delay.
 */
