 ...
 * differ depending on the {@link com.bumptech.glide.load.ResourceDecoder} using the strategy and the version of Android the code runs on. Use {@code DownsampleStrategy} as an optimization to improve
 * memory efficiency only. If you need a particular size or shape output, use an {@link com.bumptech.glide.load.Transformation} either instead or in addition to a {@code DownsampleStrategy}.
 *
 * <p>Some differences between versions of Android and {@link
 * com.bumptech.glide.load.ResourceDecoder}s are listed below, but the list is not comprehensive because {@link DownsampleStrategy} only controls its output scale value, not how that output value is
 * used.
 *
 * <p>On some versions of Android, precise scaling is not possible. In those cases, the strategies
 * can only pick between downsampling to between 1x the requested size and 2x the requested size and between 0.5x the requested size and 1x the requested size because only power of two downsampling is
 * supported. To preserve the potential for a {@link com.bumptech.glide.load.Transformation} to scale precisely without a loss in quality, all but {@link #AT_MOST} will prefer to downsample to between
 * 1x and 2x the requested size.
 */
// Public API.
...
public abstract class DownsampleStrategy {
    ...
    * of the image's dimensions is greater than or equal to the requested size. No guarantees are made about the second dimensions. This is <em>NOT</em> the same as {@link #AT_LEAST} because only one
    * dimension, not both, are greater than or equal to the requested dimensions, the other may be smaller.
    */
   ...
   /**
    * Scales, maintaining the original aspect ratio, so that one of the image's dimensions is exactly equal to the requested size and the other dimension is greater than or equal to the requested
    * size.
    *
    * <p>This method will upscale if the requested width and height are greater than the source width
    * and height. To avoid upscaling, use {@link #AT_LEAST}, {@link #AT_MOST}, or {@link #CENTER_INSIDE}.
   ...
   /**
    * Indicates the {@link com.bumptech.glide.load.resource.bitmap.DownsampleStrategy} option that will be used to calculate the sample size to use to downsample an image given the original and target
    ...
    * android.graphics.BitmapFactory.Options#inSampleSize} and a float scale factor applied after downsampling via {@link android.graphics.BitmapFactory.Options#inTargetDensity} and {@link
    * android.graphics.BitmapFactory.Options#inDensity}. Because of rounding errors the scale factor may not be applied precisely.
    *
