...
public final class Downsampler {
   /**
    * Indicates the {@link com.bumptech.glide.load.DecodeFormat} that will be used in conjunction with the image format to determine the {@link android.graphics.Bitmap.Config} to provide to {@link
    * android.graphics.BitmapFactory.Options#inPreferredConfig} when decoding the image.
    */
   ...
   /**
    * Ensure that the size of the bitmap is fixed to the requested width and height of the resource from the caller. The final resource dimensions may differ from the requested width and height, and
    * thus setting this to true may result in the bitmap size differing from the resource dimensions.
    *
    ...
    * so when Glide believes it's safe to do (when no transformations are applied). Instead, callers can set this value to {@code false} to prevent Glide from decoding hardware bitmaps if Glide is
    * unable to detect that hardware bitmaps are unsafe. For example, you should set this to {@code false} if you plan to draw it to a software {@link android.graphics.Canvas} or if you plan to
    * inspect the {@link Bitmap}s pixels with {@link Bitmap#getPixel(int, int)} or {@link Bitmap#getPixels(int[], int, int, int, int, int, int)}.
    *
   ...
   /**
    * BitmapFactory calculates the density scale factor as a float. This introduces some non-trivial error. This method attempts to account for that error by adjusting the inTargetDensity so that the
    * final scale factor is as close to our target as possible.
    */
   ...
   /**
    * Returns a Bitmap decoded from the given {@link InputStream} that is rotated to match any EXIF data present in the stream and that is downsampled according to the given dimensions and any
    ...
    * com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool} whose dimensions exactly match those of the image for the given InputStream is available, the operation is much less expensive in terms
    * of memory.
    *
