...
public class Glide implements ComponentCallbacks2 {
    ...
    * and will not be started or stopped based on lifecycle events. In general, loads should be started at the level the result will be used in. If the resource will be used in a view in a child
    * fragment, the load should be started with {@link #with(android.app.Fragment)}} using that child fragment. Similarly, if the resource will be used in a view in the parent fragment, the load
    * should be started with {@link #with(android.app.Fragment)} using the parent fragment. In the same vein, if the resource will be used in a view in an activity, the load should be started with
    * {@link #with(android.app.Activity)}}.
    *
   ...
   /**
    * Begin a load with Glide that will tied to the give {@link androidx.fragment.app.FragmentActivity}'s lifecycle and that uses the given {@link androidx.fragment.app.FragmentActivity}'s default
    * options.
    *
    * @param activity The activity to use.
    ...
    * Any {@link android.graphics.Bitmap} added to the pool must not be currently in use in any other part of the application. Any {@link android.graphics.Bitmap} added to the pool must be removed
    ...
    * worse than not pre-filling at all. Pre-filling should only be started in onCreate to avoid constantly clearing and re-filling the {@link BitmapPool}. Rotation should be carefully considered as
    * well. It may be worth calling this method only when no saved instance state exists so that pre-filling only happens when the Activity is first created, rather than on every rotation.
    *
    ...
    * MemoryCategory#HIGH} increases Glide's maximum memory usage by up to 50% and {@link MemoryCategory#LOW} decreases Glide's maximum memory usage by 50%. This method should be used to temporarily
    * increase or decrease memory usage for a single Activity or part of the app. Use {@link GlideBuilder#setMemoryCache(MemoryCache)} to put a permanent memory size if you want to change the
    * default.
    *
