...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>>
    ...
    * values that are explicitly set in the given {@link RequestOptions} object. If you need to completely reset all previously set options, create a new {@code RequestBuilder} instead of using this
    * method.
    *
   ...
   /**
    * Sets a {@link RequestListener} to monitor the resource load. It's best to create a single instance of an exception handler per type of request (usually activity/fragment) rather than pass one in
    * per request to avoid some redundant object allocation.
    *
   ...
   /**
    * Adds a {@link RequestListener}. If called multiple times, all passed {@link RequestListener listeners} will be called in order.
    *
    ...
    * this primary {@link RequestBuilder} are propagated to the given error {@link RequestBuilder}. Options like priority, override widths and heights and transitions must be applied independently to
    * the error builder.
    *
    ...
    * model passed to {@code load()} may be dangerous! Any options that were applied by the various type specific {@code load()} methods, like {@link #load(byte[])} will be copied to the error request
    * here even if the {@code model} you pass to this method doesn't match. Similary, options that would be normally applied by type specific {@code load()} methods will <em>not</em> be applied to
    * this request. If this behavior is confusing or unexpected, use {@link #error(RequestBuilder)} instead.
    */
   @NonNull
   @CheckResult
   ...
   /**
    * Loads and displays the resource retrieved by the given thumbnail request if it finishes before this request. Best used for loading thumbnail resources that are smaller and will be loaded more
    * quickly than the full size resource. There are no guarantees about the order in which the requests will actually finish. However, if the thumb request completes after the full request, the thumb
    ...
    * RequestBuilder} at position 0 has the {@link RequestBuilder} at position 1 applied as using its thumbnail method, the {@link RequestBuilder} at position 1 has the {@link RequestBuilder} at
   ...
   /**
    * Loads a resource in an identical manner to this request except with the dimensions of the target multiplied by the given size multiplier. If the thumbnail load completes before the full size
    * load, the thumbnail will be shown. If the thumbnail load completes after the full size load, the thumbnail will not be shown.
    *
    ...
    * com.bumptech.glide.request.RequestOptions#placeholder(int)} and {@link com.bumptech.glide.request.RequestOptions#error(int)}, and {@link #listener(RequestListener)} will only be used on the full
    * size load and will not be copied for the thumbnail load.
    *
    * <p>Recursive calls to thumbnail are supported.
    *
    ...
    * into Glide. If you have a custom way to obtain {@link Bitmap}s that is not supported by Glide by default, consider registering a custom {@link com.bumptech.glide.load.model.ModelLoader} or
    ...
    * com.bumptech.glide.request.RequestOptions#signature(com.bumptech.glide.load.Key)} to mixin a signature you create that identifies the data currently at the given String that will invalidate the
    * cache if that data changes. Alternatively, using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} and/or {@link com.bumptech.glide.request.RequestOptions#skipMemoryCache(boolean)}
    * may be appropriate.
    *
    ...
    * com.bumptech.glide.request.RequestOptions#signature(com.bumptech.glide.load.Key)} to mixin a signature you create based on the data at the given Uri that will invalidate the cache if that data
    * changes. Alternatively, using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} and/or {@link com.bumptech.glide.request.RequestOptions#skipMemoryCache(boolean)} may be appropriate.
    *
    ...
    * com.bumptech.glide.request.RequestOptions#signature(com.bumptech.glide.load.Key)} to mixin a signature you create that identifies the data currently in the File that will invalidate the cache if
    * that data changes. Alternatively, using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} and/or {@link com.bumptech.glide.request.RequestOptions#skipMemoryCache(boolean)} may be
    * appropriate.
    *
   ...
   /**
    * Returns a request builder that uses the {@link com.bumptech.glide.load.model.ModelLoaderFactory} currently registered or {@link Integer} to load the image represented by the given {@link
    * Integer} resource id. Defaults to {@link com.bumptech.glide.load.model.ResourceLoader} to load resource id models.
    *
    * <p>By default this method adds a version code and night mode based signature to the cache key
    * used to cache this resource in Glide. This signature is sufficient to guarantee that end users will see the most up to date versions of your Drawables, but during development if you do not
    * increment your version code before each install and you replace a Drawable with different data without changing the Drawable name, you may see inconsistent cached data. To get around this,
    * consider using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} via {@link RequestOptions#diskCacheStrategy(com.bumptech.glide.load.engine.DiskCacheStrategy)} during development,
    * and re-enabling the default {@link com.bumptech.glide.load.engine.DiskCacheStrategy#RESOURCE} for release builds.
    *
    * <p>This method will load non-{@link android.graphics.Bitmap} resources like {@link
    * android.graphics.drawable.VectorDrawable}s. Although Glide makes a best effort to apply {@link com.bumptech.glide.load.Transformation}s to these {@link Drawable}s by either extracting the
    * underlying {@link Bitmap} or by converting the {@link Drawable} to a {@link Bitmap}, Glide is still not able to transform all types of resources. Animated {@link Drawable}s cannot be transformed
    * (other than {@link com.bumptech.glide.load.resource.gif.GifDrawable}). To avoid load failures if a {@link Drawable} can't be transformed, use the optional transformation methods like {@link
   ...
   /**
    * Sets the {@link ImageView} the resource will be loaded into, cancels any existing loads into the view, and frees any resources Glide may have previously loaded into the view so they may be
    * reused.
    *
    ...
    * @param height The desired height in pixels, or {@link Target#SIZE_ORIGINAL}. This will be overridden by {@link com.bumptech.glide.request.RequestOptions#override(int, int)}} if previously
    *               called).
    * @see RequestManager#clear(Target)
    ...
    * since the width and height will be overridden by values passed to {@link RequestOptions#override(int, int)}, this method can be used whenever {@link RequestOptions} with override values are
    * applied, or whenever you want to retrieve the image in its original size.
    *
    * @see #submit(int, int)
    * @see #into(Target)
    */
   ...
   /**
    * Preloads the resource into the cache using {@link Target#SIZE_ORIGINAL} as the target width and height. Equivalent to calling {@link #preload(int, int)} with {@link Target#SIZE_ORIGINAL} as the
    * width and height.
    *
    * @return A {@link Target} that can be used to cancel the load via {@link RequestManager#clear(Target)}
    * @see #preload(int, int)
