...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
   ...
   /**
    * Applies a multiplier to the {@link com.bumptech.glide.request.target.Target}'s size before loading the resource. Useful for loading thumbnails or trying to avoid loading huge resources
    ...
    * load remote data. The animation executor has fewer threads available to it than Glide's normal executors and is only useful as a way of avoiding blocking on longer and more expensive reads for
    * critical requests like those in an animating GIF.
    *
   ...
   /**
    * Sets the {@link android.content.res.Resources.Theme} to apply when loading {@link Drawable}s for resource ids provided via {@link #error(int)}, {@link #placeholder(int)}, and {@link
    * #fallback(Drawable)}.
    *
    ...
    * android.app.Activity} used to start this load has a different {@link android.content.res.Resources.Theme}, the {@link android.content.res.Resources.Theme} provided here will override the {@link
    * android.content.res.Resources.Theme} of the {@link android.content.Context}.
    *
   ...
   /**
    * Overrides the {@link com.bumptech.glide.request.target.Target}'s width and height with the given values. This is useful for thumbnails, and should only be used for other cases when you need a
    * very specific image size.
    *
    ...
    * decoded using a decoder that cannot control the format ({@link android.media.MediaMetadataRetriever} for example), or that the decoder may choose to ignore the requested format if it can't
    * display the image (i.e. RGB_565 is requested, but the image has alpha).
    *
   ...
   /**
    * Disables the use of {@link android.graphics.Bitmap.Config#HARDWARE} in {@link Downsampler} to avoid errors caused by inspecting Bitmap pixels, drawing with hardware support disabled, drawing to
    * {@link android.graphics.Canvas}s backed by {@link Bitmap}s etc.
    *
    * <p>It's almost never safe to set {@link Downsampler#ALLOW_HARDWARE_CONFIG} to {@code true} so
    * we only provide a way to disable hardware configs entirely. If no option is set for {@link Downsampler#ALLOW_HARDWARE_CONFIG}, Glide will set the value per request based on whether or not a
    * {@link Transformation} is applied and if one is, the type of {@link Transformation} applied. Built in transformations like {@link FitCenter} and {@link
    * com.bumptech.glide.load.resource.bitmap.DownsampleStrategy.CenterOutside} can safely use {@link android.graphics.Bitmap.Config#HARDWARE} because they can be entirely replaced by scaling within
    * {@link Downsampler}. {@link Transformation}s like {@link #circleCrop()} that can't be replicated by {@link Downsampler} cannot use {@link Bitmap.Config#HARDWARE} because {@link
    * android.graphics.Bitmap.Config#HARDWARE} cannot be drawn to {@link android.graphics.Canvas}s, which is required by most {@link Transformation}s.
    */
   @NonNull
   @CheckResult
   ...
   /**
    * Applies the given {@link Transformation}s in the given order for {@link Bitmap Bitmaps} to the default types ({@link Bitmap}, {@link android.graphics.drawable.BitmapDrawable}, and {@link
    * com.bumptech.glide.load.resource.gif.GifDrawable}) and throws an exception if asked to transform an unknown type.
    *
    * <p>This will override previous calls to {@link #dontTransform()}.
    *
   ...
   /**
    * Updates this options set with any options that are explicitly set in the given {@code T} object and returns this object if {@link #autoClone()} is disabled or a new {@code T} object if {@link
    ...
    * of the set of options in this {@code T} object and the given {@code T} object that were explicitly set. If the values of any of the options conflict, the values in the returned {@code T} object
    * will be set to those in the given {@code T} object.
    */
   @NonNull
   @CheckResult
   ...
   /**
    * Similar to {@link #lock()} except that mutations cause a {@link #clone()} operation to happen before the mutation resulting in all methods returning a new Object and leaving the original locked
    * object unmodified.
    *
