...
/**
 * A general purpose size limited cache that evicts items using an LRU algorithm. By default every item is assumed to have a size of one. Subclasses can override {@link #getSize(Object)}} to change
 * the size on a per item basis.
 *
...
public class LruCache<T, Y> {
   ...
   /**
    * Sets a size multiplier that will be applied to the size provided in the constructor to put the new size of the cache. If the new size is less than the current size, entries will be evicted until
   ...
   /**
    * Returns the size of a given item, defaulting to one. The units must match those used in the size passed in to the constructor. Subclasses can override this method to return sizes in various
    * units, usually bytes.
    *
    ...
    * where {@link #getSize(Object)} returns different values for the same object when called at different times, the size value is acquired in {@code put} and retained until the item is evicted,
    * replaced or removed.
    *
    * <p>If {@code item} is null the behavior here is a little odd. For the most part it's similar to
    * simply calling {@link #remove(Object)} with the given key. The difference is that calling this method with a null {@code item} will result in an entry remaining in the cache with a null value
    * and 0 size. The only real consequence is that at some point {@link #onItemEvicted(Object, Object)} may be called with the given {@code key} and a null value. Ideally we'd make calling this
    * method with a null {@code item} identical to {@link #remove(Object)} but we're preserving this odd behavior to match older versions :(.
    *
